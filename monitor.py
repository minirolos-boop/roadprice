import os, argparse, logging, json, sqlite3
...
def fetch_travels():
    ...
    # Auth optionnelle : si un jeton existe, on l'ajoute
...
def weekly_kpis(df):
    ...
    s = df["month"].dropna()
    out["depart_by_month"] = s.value_counts().sort_index().to_dict()
    return out

def cheapest_by_destination(df):
    base = df.dropna(subset=["destination_label","price_eur"]).copy()
    if base.empty:
        return pd.DataFrame(columns=["title","country_name","price_eur","url"])\
                 .set_index(pd.Index([], name="destination_label"))
    idx = base.groupby("destination_label")["price_eur"].idxmin()
    return base.loc[idx, ["destination_label","title","country_name","price_eur","url"]]\
              .set_index("destination_label")

def weekly_diff(df_curr, df_prev):
...
def find_alerts(weekly_diff_df, pct_threshold=0.10, abs_threshold=150.0):
    x = weekly_diff_df.copy()
    x["flag"] = (
        (x["delta_pct"].abs() > pct_threshold) |
        (x["delta_abs"].abs() > abs_threshold)
    )
...
def persist_sqlite(df_curr, wk_kpis, wk_diff, mo_kpis, mo_diff, alerts, db_path, run_date):
    ...
    try:
        ...
        pd.DataFrame([{"run_date": run_date, **wk_kpis}]).to_sql("weekly_kpis", conn, if_exists="append", index=False)
...
def main():
    ...
    ap.add_argument("--alert-pct", type=float, default=float(os.getenv("ALERT_PCT", 0.10)), help="Seuil variation %% (ex: 0.10 = 10%)")
    ap.add_argument("--alert-eur", type=float, default=float(os.getenv("ALERT_EUR", 150.0)), help="Seuil variation absolue en €")
    args = ap.parse_args()
    ...
    # Charger dernier snapshot pour le diff hebdo (si base existe)
    df_prev = None
