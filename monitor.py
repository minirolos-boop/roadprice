...
# -------------------- Fetch + normalize --------------------
def fetch_travels():
    """Récupère la liste des voyages (destinations)."""
    s = requests.Session()
...
def fetch_tours(slug: str):
    """Récupère tous les départs (tours) d’un voyage via son slug."""
    url = API_TOURS.format(slug=slug)
    r = requests.get(url, timeout=30)
    if r.status_code != 200:
        logging.warning("Échec fetch tours slug=%s (%s)", slug, r.status_code)
        return []
    data = r.json()
...
def normalize(travels):
    """Normalise en lignes : chaque ligne = un départ précis (tour_id)."""
    rows = []
    for t in travels:
        slug = t.get("slug")
        tours = fetch_tours(slug) if slug else []
        for tour in tours:
            ...
            rows.append(
                {
                    "tour_id": tour.get("id"),  # id unique du départ
                    "slug": slug,
                    "url": f"https://www.weroad.fr/destinations/{slug}" if slug else None,
                    "url_precise": f"https://www.weroad.fr/destinations/{slug}/{tour.get('id')}" if slug and tour.get("id") else None,
                    ...
                    "min_price_eur": min_p,
                    "max_price_eur": max_p,
                    "best_starting_date": g(tour, ["startingDate"]),
                    "best_ending_date": g(tour, ["endingDate"]),
...
def weekly_kpis(df: pd.DataFrame) -> dict:
    ...
    for c in ["price_eur", "base_price_eur", "discount_value_eur", "discount_pct"]:
        series = pd.to_numeric(df[c], errors="coerce") if c in df else pd.Series(dtype=float)
...
def weekly_diff(df_curr: pd.DataFrame, df_prev: pd.DataFrame | None) -> pd.DataFrame:
    """Compare prix min par destination entre run courant et précédent."""
    if df_curr.empty:
        return pd.DataFrame()

    # Prix min par destination (courant)
    L = df_curr.dropna(subset=["destination_label", "price_eur"]).copy()
    L = L.loc[L.groupby("destination_label")["price_eur"].idxmin(), [
        "destination_label", "title", "price_eur", "url", "url_precise"
    ]].rename(columns={
        "title": "title_curr",
        "price_eur": "price_eur_curr"
    })

    # Prix min par destination (précédent)
    if df_prev is None or df_prev.empty:
        R = pd.DataFrame(columns=["destination_label", "title_prev", "price_eur_prev", "url_prev", "url_precise_prev"])
    else:
        R = df_prev.dropna(subset=["destination_label", "price_eur"]).copy()
        R = R.loc[R.groupby("destination_label")["price_eur"].idxmin(), [
            "destination_label", "title", "price_eur", "url", "url_precise"
        ]].rename(columns={
            "title": "title_prev",
            "price_eur": "price_eur_prev"
        })
    ...
    # Choisir l’URL précise si dispo
    diff["url"] = diff["url_precise"].fillna(diff["url"])
